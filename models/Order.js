...
class OrderModel {
  ...
  try {
    ...
    console.error(`Lỗi khi đếm sản phẩm theo trạng thái ${paymentStatus}:`, error);
    throw error;
  }
}
async getDashboardStats(date = null) {
  ...
  async getTotalRevenueByDateRange(startDate, endDate) {
    try {
      ...
      console.error(`❌ Lỗi khi lấy doanh thu từ ${startDate} đến ${endDate}:`, error);
      throw error;
    }
  }
...
async getTotalOrdersByDateRange(startDate, endDate) {
  try {
    ...
    console.error(`❌ Lỗi khi lấy tổng đơn hàng từ ${startDate} đến ${endDate}:`, error);
    throw error;
  }
}
...
async getTotalCustomersByDateRange(startDate, endDate) {
  try {
    ...
    console.error(`❌ Lỗi khi lấy tổng khách hàng từ ${startDate} đến ${endDate}:`, error);
    throw error;
  }
}
async getRevenueChartData(startDate, endDate) {
  try {
    console.log(`📊 Lấy dữ liệu biểu đồ doanh thu từ ${startDate} đến ${endDate}`);
